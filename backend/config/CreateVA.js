...
export async function createTransactionVAEgate (userOrder, orderId){
    console.log(userOrder)
    console.log(`${process.env.URL_BACKEND}/api/orders/webhook/va`)
    const body = {
        ...
        bank_code: userOrder.bank_code,
        customer_name: userOrder.name,
        remark: userOrder.remark,
        url_callback: `${process.env.URL_BACKEND}/api/orders/webhook/va`,
        identifier_id: orderId,
    }
   try {
       const resultVa = await axios.post(`${process.env.EGATE_HOST}/partner/create/va`,
           body,
           {
               headers: {
...
async function generateSignature(jsonBody) {
    const request_target = '/partner/create/va';
